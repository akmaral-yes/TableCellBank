...
def draw_rectangles(img, rect_list, color, thickness=3):
    """
...
def draw_filled_rect(tables_path, tables_rect_path, images_list=[]):
    """
...
def draw_annotations(annotations, tables_path, tables_gt_path, images_list=[],
                     mask=True):
    """
    Draw on the original table images the bounding-boxes of each column/row,
    and segmentation masks
    """
    # Unless specific images indicated, build for all table images
    if len(images_list) == 0:
        images_list = os.listdir(tables_path)
    for table_name in images_list:
        print("table_name: ", table_name)
        ...
        number_of_instances = len(annotations[table_name])
        # Draw annotation column by column
        for i in range(0, number_of_instances):
            bbox = annotations[table_name][i]["bbox"]
            polys = annotations[table_name][i]["segmentation"]
            x1, y1, x2, y2 = bbox
...
def draw_predictions_gt(table_images_path, cells_gt_dict, cells_predicted_dict,
                        tables_predictions_gt_path, table_list=[]):
    """
    ...
    for table_name in table_names:
        ...
        table_image = draw_rectangles(
            table_image, cells_predicted_list, color_predicted)
